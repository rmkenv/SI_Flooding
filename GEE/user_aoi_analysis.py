"""
Flood Detection and Analysis using Google Earth Engine Python API
================================================================

This script performs flood detection using Sentinel-2 optical and Sentinel-1 SAR data,
calculates water indices, and exports results for machine learning analysis.
"""
...
max_images_pre = 50  # Max images for pre-event composite
max_images_post = 20  # Max images for post-event composite
...
AOI = ee.Geometry.Rectangle([-77.3, 38.8, -76.0, 39.7])

# Create a date string for file naming
date_string = post_start.replace('-', '')

print(f"AOI defined: {AOI.getInfo()}")
...
def check_valid_bands(image):
    """Check if an image has all required bands for water indices."""
    image = ee.Image(image)
    bn = image.bandNames()
    
    # Check if each band exists
    has_b3 = bn.contains('B3')
    has_b4 = bn.contains('B4')
    has_b8 = bn.contains('B8')
    has_b11 = bn.contains('B11')
    has_b12 = bn.contains('B12')
    
    # Convert booleans to numbers and sum
    val_b3 = ee.Algorithms.If(has_b3, ee.Number(1), ee.Number(0))
    val_b4 = ee.Algorithms.If(has_b4, ee.Number(1), ee.Number(0))
    val_b8 = ee.Algorithms.If(has_b8, ee.Number(1), ee.Number(0))
    val_b11 = ee.Algorithms.If(has_b11, ee.Number(1), ee.Number(0))
    val_b12 = ee.Algorithms.If(has_b12, ee.Number(1), ee.Number(0))
    
    # Cast to numbers and sum
    sum_checks = (ee.Number(val_b3)
                  .add(ee.Number(val_b4))
                  .add(ee.Number(val_b8))
                  .add(ee.Number(val_b11))
                  .add(ee.Number(val_b12)))
    
    # Check if all 5 bands are present
    is_valid = sum_checks.eq(5)
...
def composite_monthly(collection, start, end):
    """Create monthly median composites from an image collection."""
    ...
    # Filter out empty placeholder images and clip to AOI
...
print(f"Pre-monthly collection size: {pre_monthly.size().getInfo()}")
print(f"Post-monthly collection size: {post_monthly.size().getInfo()}")

# Create dummy image with expected bands
...
pre = ee.Image(ee.Algorithms.If(
    ...
    pre_monthly.median().clip(AOI),
    create_dummy_image()
))

post = ee.Image(ee.Algorithms.If(
    post_monthly.size().gt(0),
    post_monthly.median().clip(AOI),
    create_dummy_image()
))

print("Pre and post composite images created")
...
s1_water = s1_composite.select('VV').lt(vv_threshold)
...
# Mask out urban areas from the flood map
...
not_permanent_water = permanent_water_mask.Not()
...
def export_samples():
    """Prepare and export stratified samples for machine learning."""
    samples = (flood_final.rename('flood')
               .addBands(ndwi_post)
               .addBands(mndwi_post)
               .addBands(awei_post)
               .addBands(s1_water)
...
def export_predictor_bands():
    """Export predictor bands for offline ML prediction."""
    # Rename bands and cast to Float32 for consistency
    ndwi_post_renamed = ndwi_post.rename('ndwi_post').toFloat()
    mndwi_post_renamed = mndwi_post.rename('mndwi_post').toFloat()
    awei_post_renamed = awei_post.rename('awei_post').toFloat()
    s1_vv_renamed = s1_composite.select('VV').rename('VV').toFloat()
    elevation_renamed = elevation.rename('elevation').toFloat()
    slope_renamed = slope.rename('slope').toFloat()
    landcover_renamed = landcover.rename('Map').toFloat()
    jrc_seasonality_renamed = seasonality.rename('jrc_seasonality').toFloat()
    ...
    task = ee.batch.Export.image.toDrive(
        ...
        description=f'predictor_bands_for_ml_{date_string}',
        folder='GEE_Flood_Predictors',
        fileNamePrefix=f'predictors_{date_string}',
        region=AOI,
        scale=scale,
        maxPixels=1e9,
        fileFormat='GeoTIFF'
...
if __name__ == "__main__":
    print("Starting flood detection and analysis...")
    ...
    print("Check your Google Drive 'GEE_Flood' and 'GEE_Flood_Predictors' folders for results.")
    print("You can monitor task progress at: https://code.earthengine.google.com/tasks")
    
    # Optional: Print some basic statistics
    try:
        print(f"\nBasic Statistics:")
        print(f"AOI area: {AOI.area().divide(1e6).getInfo():.2f} km²")
        print(f"Pre-event images used: {pre_monthly.size().getInfo()}")
        print(f"Post-event images used: {post_monthly.size().getInfo()}")
        print(f"S1 images used: {s1.size().getInfo()}")
